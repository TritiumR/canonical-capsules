...
def vis_relation(x, prev_x, point_id, similarity=None, idx=0, vis_fn="temp.png"):
    pts = x[0].transpose(1, 0).cpu().numpy()
    prev_pts = prev_x[0].transpose(1, 0).cpu().numpy()
    label_map = None
    prev_label_map = None
    if similarity is not None:
        label_map = torch.zeros(len(similarity))
        label_map[point_id] = 100
        prev_label_map = 100 * torch.clamp(similarity, min=0).cpu().numpy()
    prev_vis_fn = os.path.join(vis_fn, f"prev_relation_{idx}")
    vis_fn = os.path.join(vis_fn, f"relation_{idx}")
    vis_pts_relation(pts, label_map, fn=vis_fn)
    vis_pts_relation(prev_pts, prev_label_map, fn=prev_vis_fn)
...
def vis_pts_relation(pts, label_map, fn="temp.png", marker=".", alpha=0.9):
    # pts (n, d): numpy, d-dim point cloud
    # label_map (n, ): numpy or None
    # fn: filename of visualization
    assert pts.shape[1] in [2, 3]
    if pts.shape[1] == 2:
        ...
        ys = pts[:, 1]
        if label_map is not None:
            plt.scatter(xs, ys, c=label_map, cmap="Blues", marker=".", alpha=0.9, edgecolor="none")
        else:
            plt.scatter(xs, ys, c="grey", alpha=0.8, edgecolor="none")
        # save
        plt.xlim(-1,1)
        ...
        zs = pts[:, 2]
        if label_map is not None:
            ax.scatter(xs, ys, zs, c=label_map, cmap="plasma", marker=marker, alpha=alpha)
        else:
            ax.scatter(xs, ys, zs, marker=marker, alpha=alpha, edgecolor="none")
